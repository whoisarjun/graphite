...
data_to_process = [
    ('./2ND_BATCH_DATA/CROHME_test_2011/', './2ND_DATA/2011_test/'),
    ('./2ND_BATCH_DATA/CROHME_training_2011/', './2ND_DATA/2011_train/'),
    ('./2ND_BATCH_DATA/MatricesTest2014/', './2ND_DATA/2014_test'),
    ('./2ND_BATCH_DATA/MatricesTrain2014/', './2ND_DATA/2014_train/'),
    ('./2ND_BATCH_DATA/testData_2012/', './2ND_DATA/2012_test/'),
    ('./2ND_BATCH_DATA/TestINKML_2013/', './2ND_DATA/2013_test/'),
    ('./2ND_BATCH_DATA/trainData_2012_part1/', './2ND_DATA/2012_train_1/'),
    ('./2ND_BATCH_DATA/trainData_2012_part2/', './2ND_DATA/2012_train_2/'),
    ('./2ND_BATCH_DATA/TrainINKML_2013/', './2ND_DATA/2013_train')
]
...
base_cleaned_folder = './2ND_BATCH_CLEANED'
os.makedirs(base_cleaned_folder, exist_ok=True)

def convert_single_file(args):
    inkml_path, output_png_path = args
    try:
        inkml2img(inkml_path, output_png_path)
        return (inkml_path, True, None)
    except Exception as e:
        return (inkml_path, False, str(e))

for src_folder, _ in data_to_process:
    folder_name = Path(src_folder).stem
    out_folder = os.path.join(base_cleaned_folder, folder_name)
    os.makedirs(out_folder, exist_ok=True)

    files = [f for f in os.listdir(src_folder) if f.endswith('.inkml')]
    args = []
    for filename in files:
        inkml_path = os.path.join(src_folder, filename)
        output_png_name = filename.replace('.inkml', '.png')
        output_png_path = os.path.join(out_folder, output_png_name)
        args.append((inkml_path, output_png_path))

    with Pool(processes=cpu_count()) as pool:
        for inkml_path, success, error in tqdm(pool.imap_unordered(convert_single_file, args), total=len(args)):
            if success:
                print(f"✔️ Converted: {Path(inkml_path).name}")
            else:
                print(f"❌ Failed on {Path(inkml_path).name}: {error}")
